...
def index(request):
    ...
    if request.user.is_superuser:
        return redirect('admin:index')  # Redirect to admin index using named URL
...
def get_random_date():
    # Generate a random date within a reasonable range, for example, within the last year.
...
def t_report(request, assign_id):
    ...
    for stud in ass.class_id.student_set.all():
        ...
        # Append a random date to the student's StudentCourse object
        random_date = get_random_date()
        a.random_date = random_date
        sc_list.append(a)
